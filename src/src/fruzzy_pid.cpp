...
//实现模糊控制
float FuzzyPID::realize(float t,float a,float Kp1,float Kd1)
{
    ...
    // if (abs(e1 - e2) > (COLSIMAGE / 10))
    // {
    //     e1 = e1 > e2 ? e2 + (COLSIMAGE / 10) : e2 - (COLSIMAGE / 10);//限幅
    // }
    ...
    ofstream file_pid;
    file_pid.open("/home/edgeboard/Desktop/sasu2024/src/src/pid.txt",std::ios ::app);
    file_pid<<Kp<<" "<<Ki<<" "<<Kd<<endl;
    file_pid.close();
    //cout<<"Kp="<<Kp<<endl;
    //cout<<"Ki="<<Ki<<endl;
    //cout<<"Kd="<<Kd<<endl;
